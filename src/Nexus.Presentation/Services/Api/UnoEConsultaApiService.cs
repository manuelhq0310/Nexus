...
public class UnoEConsultaApiService : ApiServiceBase
{
    ...
    {
        var response = await Http.PostAsJsonAsync("api/unoe/consultas/configuraciones", dto);
    ...
    {
        var response = await Http.PutAsJsonAsync("api/unoe/consultas/configuraciones", dto);
        await EnsureSuccessAsync(response);
